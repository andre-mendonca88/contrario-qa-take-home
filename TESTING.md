...
- CI runs `npm run typecheck` (`tsc --noEmit`) as its own step, before any test
  step. It was added after a real type error in a test file passed locally
  under `npm run test:api` but failed CI — ts-jest's diagnostics, at least as
  configured here, are not a reliable substitute for running `tsc` directly.
